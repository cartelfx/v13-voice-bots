...
VoiceChannel.prototype.hasStaff = function(checkMember = false) {
    if(this.members.some(m => (checkMember !== false ? m.user.id !== checkMember.id : true) && !m.user.bot && m.roles.highest.position >= m.guild.rolüBul(this.voiceSys.Staff).position)) return true;
    return false;
}

VoiceChannel.prototype.getStaffs = function(checkMember = false) {
    return this.members.filter(m => (checkMember !== false ? m.user.id !== checkMember.id : true) && !m.user.bot && m.roles.highest.position >= m.guild.rolüBul(this.voiceSys.Staff).position).size
}
    ...
    if(
        ...
        (
           ...
           this.roles.highest.position >= this.guild.roles.cache.get(this.voiceSys.Staff).position
        )
    ) return true;
    return false;
}
